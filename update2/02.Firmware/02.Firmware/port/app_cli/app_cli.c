...
static int32_t cli_ping_gw(p_shell_context_t context, int32_t argc, char **argv);
...
static int32_t cli_ping_gw(p_shell_context_t context, int32_t argc, char **argv)
{
    extern void on_button_event_cb(int button_pin, int event, void *data);
    if (strstr(argv[1], "alarm"))
    {
        on_button_event_cb(0, APP_BTN_EVT_PRESSED, NULL);
    }
    else if (strstr(argv[1], "normal"))
    {
        on_button_event_cb(0, APP_BTN_EVT_RELEASED, NULL);
    }
