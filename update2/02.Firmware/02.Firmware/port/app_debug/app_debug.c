...
static uint8_t m_level = APP_DBG_LV;
static app_dbg_output_cb_t m_write_cb[APP_DBG_NUMBER_OF_DBG_PORT];
static uint8_t number_of_callback = 0;
static app_dbg_get_timestamp_ms_cb_t m_get_ms;
static app_dbg_lock_cb_t m_lock_cb;
...
static lwrb_t m_ringbuffer_dbg_isr;
static uint8_t m_isr_buffer_size[DBG_ISR_RINGBUFFER_SIZE];
#endif

#ifdef APP_DBG_HAS_FLOAT
...
void app_dbg_init(app_dbg_get_timestamp_ms_cb_t get_ms, app_dbg_lock_cb_t lock_cb)
{
    ...
    m_lock_cb = lock_cb;
#if DBG_ISR_ENABLE
    lwrb_init(&m_ringbuffer_dbg_isr, &m_isr_buffer_size, DBG_ISR_RINGBUFFER_SIZE);
#endif
}
...
void app_dbg_putc(uint8_t data)
{
    for (uint8_t index = 0; index < APP_DBG_NUMBER_OF_DBG_PORT; index++)
    {
        if (m_write_cb[index])
        {
...
{
    lwrb_write(&m_ringbuffer_dbg_isr, &c, 1);
}

static inline void put_byte_in_isr(uint8_t data)
{
    lwrb_write(&m_ringbuffer_dbg_isr, &data, 1);
}
...
static inline void put_string_in_isr(char *s)
{
    ...
    {
        char *null_ptr = "NULL";
        lwrb_write(&m_ringbuffer_dbg_isr, null_ptr, 4);
    }
}
...
// #if 0
//     int32_t n;
//     char *p = &m_dbg_buffer[0];
//     int size = SEGGER_RTT_PRINTF_BUFFER_SIZE;
//     int time_stamp_size;

//     p += sprintf(m_dbg_buffer, "<%u>: ", app_dbg_get_ms());
//     time_stamp_size = (p-m_dbg_buffer);
...
//             if (m_write_cb[index])
//             {
//                 m_write_cb[index](0, m_dbg_buffer, n + time_stamp_size);
//             }
//         }
...
//         if (d == 'c')
//         {
//             app_dbg_putc((char)va_arg(arp, int32_t));
//             continue;
//         }
...
void app_dbg_print_raw(const char *fmt, ...)
{
    ...
    repeat:
        ...
        case 'c':
            ...
            app_dbg_putc((unsigned char)va_arg(args, int));
            nb_of_bytes++;
...
static void simple_print_hex(uint8_t hex_value)
{
    ...
    app_dbg_putc(hex_str[(hex_value >> 4) & 0x0F]);
    app_dbg_putc(hex_str[(hex_value)&0x0F]);
}
...
void app_dbg_isr_ringbuffer_flush(void)
{
    ...
    uint8_t tmp;
    while (lwrb_read(&m_ringbuffer_dbg_isr, &tmp, 1))
    {
        app_dbg_putc(tmp);
    }
...
void app_dbg_register_callback_print(app_dbg_output_cb_t callback)
{
    if (m_lock_cb)
    {
        m_lock_cb(true, 0xFFFFFFFF);
    ...
    if (callback)
    {
        for (uint8_t func_count = 0; func_count < APP_DBG_NUMBER_OF_DBG_PORT; func_count++)
        {
...
void app_dbg_unregister_callback_print(app_dbg_output_cb_t callback)
{
    if (m_lock_cb)
    {
        m_lock_cb(true, 0xFFFFFFFF);
