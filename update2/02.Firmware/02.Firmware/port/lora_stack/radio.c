...
extern void RadioControlTxEnAntennaSwitch(bool tx_en);
...
void RadioInit(RadioEvents_t *events)
{
    APP_DBG_INF("Radio initialize\r\n");
//    APP_DBG_VERBOSE("Radio initialize\r\n");
    RadioEvents = events;
                        ...
                        uint8_t hopPeriod, bool iqInverted, uint32_t timeout)
{
    ...
    switch(modem)
    {
        ...
        case MODEM_LORA:
            ...
            {
                SX126x.ModulationParams.Params.LoRa.LowDatarateOptimize = 0x01;
                APP_DBG_VERBOSE("Low datarate optimized\r\n");
            }
            else
            {
                SX126x.ModulationParams.Params.LoRa.LowDatarateOptimize = 0x00;
                APP_DBG_VERBOSE("Low datarate not optimize\r\n");
            }
...
void RadioSend(uint8_t *buffer, uint8_t size)
{
//    DEBUG_PRINTF("Radio send %d bytes\r\n", size);
    SX126xSetDioIrqParams(IRQ_TX_DONE | IRQ_RX_TX_TIMEOUT,
                           IRQ_TX_DONE | IRQ_RX_TX_TIMEOUT, // dio1Mask
    ...
    RadioControlTxEnAntennaSwitch(true);
    SX126xSetTxParams(22, RADIO_RAMP_200_US);
    SX126xSetPacketParams(&SX126x.PacketParams);
//    DEBUG_PRINTF("Set antenna TX en\r\n");

    uint32_t time_on_air = RadioTimeOnAir(MODEM_LORA, size);
    
    time_on_air *= (2000);
    SX126xSendPayload(buffer, size, time_on_air);
//    DEBUG_PRINTF("Time on air %u us\r\n", time_on_air);
...
//    DEBUG_PRINTF("Set antenna TX disable, RX en\r\n");
    RadioControlTxEnAntennaSwitch(false);
    if (RxContinuous == true)
    {
        APP_DBG_INF("Radio rx wait forever\r\n", timeout);
        SX126xSetRx(0xFFFFFF); // Rx Continuous
    }
    else
    {
        APP_DBG_INF("Radio rx %ums\r\n", timeout);
...
void RadioOnTxTimeoutIrq(void)
{
    APP_DBG_WRN("RadioOnTxTimeoutIrq\r\n");
...
void RadioOnRxTimeoutIrq(void)
{
    APP_DBG_WRN("RadioOnRxTimeoutIrq\r\n");
...
void RadioDebugIrq(uint16_t irg)
{
    if ((irg & IRQ_TX_DONE) == IRQ_TX_DONE)
    {
        APP_DBG_INF("IRQ TX done\r\n");
        RadioControlTxEnAntennaSwitch(false);
//        DEBUG_PRINTF("Set antenna TX disable, rx enable\r\n");
    }
    
    if ((irg & IRQ_RX_DONE) == IRQ_RX_DONE)
    {
//        DEBUG_PRINTF("Set antenna TX disable, rx enable\r\n");
        APP_DBG_INF("IRQ_RX_DONE\r\n");
        RadioControlTxEnAntennaSwitch(false);
    }
    
    
    if ((irg & IRQ_PREAMBLE_DETECTED) == IRQ_PREAMBLE_DETECTED)
    {
        APP_DBG_INF("IRQ_PREAMBLE_DETECTED\r\n");
    }
    
    if ((irg & IRQ_SYNCWORD_VALID) == IRQ_SYNCWORD_VALID)
    {
        APP_DBG_INF("IRQ_SYNCWORD_VALID\r\n");
    }
    
    if ((irg & IRQ_HEADER_VALID) == IRQ_HEADER_VALID)
    {
        APP_DBG_INF("IRQ_HEADER_VALID\r\n");
    }
    
    if ((irg & IRQ_HEADER_ERROR) == IRQ_HEADER_ERROR)
    {
        APP_DBG_WRN("IRQ_HEADER_ERROR\r\n");
    }

    
    if ((irg & IRQ_CRC_ERROR) == IRQ_CRC_ERROR)
    {
        APP_DBG_WRN("IRQ_CRC_ERROR\r\n");
    }
    
    if ((irg & IRQ_CAD_DONE) == IRQ_CAD_DONE)
    {
        APP_DBG_INF("IRQ_CAD_DONE\r\n");
    }
    
    if ((irg & IRQ_CAD_ACTIVITY_DETECTED) == IRQ_CAD_ACTIVITY_DETECTED)
    {
        APP_DBG_INF("IRQ_CAD_ACTIVITY_DETECTED\r\n");
    }
    
    if ((irg & IRQ_RX_TX_TIMEOUT) == IRQ_RX_TX_TIMEOUT)
    {
        RadioControlTxEnAntennaSwitch(false);
        APP_DBG_WRN("IRQ_RX_TX_TIMEOUT\r\n");
    }
}
...
void RadioIrqProcess(void)
{
	uint16_t irqRegs = 0;
    if (IrqFired == true)
//    if (GPIO_ReadInputDataBit(RADIO_DIO1_PORT, RADIO_DIO1_PIN)==Bit_SET)       // READ DIO1
//    if (gpio_input_bit_get(GPIOB, GPIO_PIN_1))
    {
        ...
        if ((irqRegs & IRQ_RX_DONE) == IRQ_RX_DONE)
        {
            ...
            {
                memset(RadioRxPayload, 0, sizeof(RadioRxPayload));
                SX126xGetPayload(RadioRxPayload, &size , 255);
                SX126xGetPacketStatus(&RadioPktStatus);
                if ((RadioEvents != NULL) && (RadioEvents->RxDone != NULL))
                {
                    RadioEvents->RxDone(RadioRxPayload, 
                                        size, 
        ...
        {
            //__NOP();
            if ((RadioEvents != NULL) && (RadioEvents->PreambleDetect != NULL))
            {
                RadioEvents->PreambleDetect();
            }
        }
