...
{
    m_master_msg_ctx.msg.header.type.name.node_id = 0;
...
void lora_task_poll()
{
    ...
    {
        if (systime_is_timer_elapse(&m_systime_auto_reset_lora_module, m_cfg.period_reset_ms_when_moderm_good))
        {
    ...
    if (!APP_LORA_TX_IS_IDLE())
    {
            ...
            && systime_is_timer_elapse(&m_systime_tx_timeout, (m_master_msg_ctx.timeout_ms + random_timeout_ms))) 
        {
            ...
            if (m_tx_retry_counter-- > 0)
            {
                ...
                APP_DBG_ERR("[%d] LORA TX retry %d times, tx timeout %d\r\n", m_cfg.get_ms(), 
                                                                                m_tx_retry_counter, 
...
static void on_lora_frame_callback(uint8_t *data, uint8_t size)
{
    ...
    if (get_rx_payload(data, size, &header, &body, &payload_len))
    {
        ...
        APP_DBG_INF("Network id %d, msg type %d, device type %s, node id %d, from %s\r\n", 
                    header->network_id,
                    header->type.name.msg_type,
                    app_lora_get_device_type_str(header->type.name.device_type),
                    header->type.name.node_id,
            ...
            && header->network_id)
        {
            if (m_cfg.event)
            {
                ...
                rx_msg.node_id = header->type.name.node_id;
                rx_msg.payload = body;
                        ...
                        void *arg)
{
    ...
    if (!APP_LORA_TX_RX_IS_IDLE())
    {
        APP_DBG_INF("[%d] Lora TXRX is busy\r\n", m_cfg.get_ms());
        goto end;
    }
    ...
    m_master_msg_ctx.msg.header.type.name.node_id = data->node_id;
    ...
    APP_DBG_INF("[%d] Sending lora packet size %d, node id %d\r\n", 
                    m_cfg.get_ms(), 
                    m_master_msg_ctx.msg.len, 
                    m_master_msg_ctx.msg.header.type.name.node_id);
...
static bool app_lora_master_retry_tx(app_lora_retry_msg_ctx_t *msg)
{
    ...
    {
        APP_DBG_ERR("[%d] Lora rx is busy\r\n", m_cfg.get_ms());
        return false;
    }

    if (!lora_hal_is_device_on_bus())
    {
        APP_DBG_ERR("[%d] Lora chip is not detected\r\n", m_cfg.get_ms());
    }

    APP_DBG_INF("[%d] Re-sending lora packet size %d\r\n", m_cfg.get_ms(), m_master_msg_ctx.msg.len);
                        ...
                        void *arg)
{
    ...
    APP_DBG_INF("[%d] Sending lora packet size %d, node id %d, from %s to %s\r\n", 
                    m_cfg.get_ms(), 
                    m_master_msg_ctx.msg.len, 
                    m_master_msg_ctx.msg.header.type.name.node_id,
