...
void lora_hal_power_poll(void)
{
    ...
    case LORA_PREPARE_OFF:
    {
        ...
        if (systime_is_timer_elapse(&m_timer_power_monitor, 10)) // 10ms
        {
    ...
    case LORA_POWER_OFF:
    {
        ...
        if (systime_is_timer_elapse(&m_timer_power_monitor, 20)) // 10ms
        {
...
void lora_hal_packet_fsm_poll(void)
{
    ...
    {   
        //lora_hal_enter_receive_mode();
        Radio.IrqProcess( ); // Process Radio IRQ
//        static uint32_t count = 0;
//        if (count++ == 200)
//        {
//            count = 0;
////            uint16_t irq = sx126x_GetIrqStatus();
////            if (irq)
////            {
////                APP_DBG_INF("IRQ status 0x%04X\r\n", irq);
////            }
//        }
    }
        break;
...
static void on_lora_phy_new_tx_cplt(void)
{
    ...
    m_hal_conf.delay_ms(100);   // Delay de tranh noise
   // #warning "HuyTV them delay de tranh noise"
    Radio.Standby();
    Radio.RxBoosted(RX_TIMEOUT_VALUE);
